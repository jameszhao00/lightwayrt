	...
	const Scene* scene)
{
	...
	//direct lighting
	{
		...
		{
			color light_T;
			Hit<World> light_vtx = scene->sample_light(eye_vtx1.position, 
				rand2(RandomKey(linid(), pass_data->iteration_idx), RandomCounter(i, 1)), &light_T);
	...
	//indirect lighting
	//? maybe ? klemens mcmc... but with the first 2 eye verts locked
	{
		const int RANDOM_PAIR_PER_CYCLE = 2;
 		int bounce = 0;
 		int samples = 0;
 		...
 		for(int i = 0; i < NumCycles; i++)
 		{
			...
			if(eye_T.x == -1)
			{
 			...
 			next_wi(*scene, rand2(RandomKey(linid(), pass_data->iteration_idx), 
				RandomCounter(RANDOM_PAIR_PER_CYCLE * i + 0, 0)), 
			...
			if(hit && scene->materials[eye_vtx.material_id].type != eSpecular)// && !is_SD_path)
			{
				color light_T;
				//connect
				Hit<World> light_vtx = scene->sample_light(eye_vtx.position, 
					rand2(RandomKey(linid(), pass_data->iteration_idx), 
					RandomCounter(RANDOM_PAIR_PER_CYCLE * i + 1, 1)), &light_T);
